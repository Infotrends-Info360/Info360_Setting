...
@Path("/Ppermission")
public class Ppermission_Servlet {
	...
	public Response postFromPath(@FormParam("dbid") int dbid)
		...
		try {
			...
			List<CFG_person> cfg_personlist = maintainService
					.Ppermission(cfg_person);
			...
			if (cfg_person.getDbid() != 0) {
				...
				for (int i = 0; i < cfg_personlist.size(); i++) {
					...
					PersonJsonObject.put("dbid", cfg_personlist.get(i)
							.getDbid());
					PersonJsonObject.put("createdatetime", cfg_personlist
							.get(i).getCreatedatetime());
					PersonJsonObject.put("account", cfg_personlist.get(i)
							.getAccount());
					PersonJsonObject.put("password", cfg_personlist.get(i)
							.getPassword());
					PersonJsonObject.put("first_name", cfg_personlist.get(i)
							.getFirst_name());
					PersonJsonObject.put("last_name", cfg_personlist.get(i)
							.getLast_name());
					PersonJsonObject.put("user_name", cfg_personlist.get(i)
							.getUser_name());
					PersonJsonObject.put("employee_id", cfg_personlist.get(i)
							.getEmployee_id());
					PersonJsonObject.put("emailaddress", cfg_personlist.get(i)
							.getEmailaddress());
					PersonJsonObject.put("state", cfg_personlist.get(i)
							.getState());
				...
				if (1 == 1) {
					...
					cfg_group_person.setPerson_dbid(cfg_personlist.get(0)
							.getDbid());
					...
					for (int i = 0; i < cfg_person_grouplist.size(); i++) {
						...
						cfg_group.setDbid(cfg_person_grouplist.get(i)
								.getGroup_dbid());

						List<CFG_group> cfg_grouplist = maintainService
								.query_Group(cfg_group);
						JSONObject GroupJsonObject = new JSONObject();
						GroupJsonObject.put("dbid", cfg_grouplist.get(0)
								.getDbid());
						GroupJsonObject.put("name", cfg_grouplist.get(0)
								.getName());
